...
aws s3 cp "$DISCOUNT_HOME/target/scala-2.12/Discount-assembly-4.0.0.jar" $BUCKET/
...
COMMAND=( \
#  --conf $SPLIT \
  --class com.jnpersson.discount.spark.Discount $BUCKET/Discount-assembly-4.0.0.jar $*)
